/// Cute Fantasy Free pack POC — individual tile sprites + assembled house + player.
/// Run with: cargo run --bin poc_cutefantasy
...
use the_nature_of_things::render::draw_cutefantasy::{FantasyTextures, TS, draw_world_cutefantasy, cam_max_x, cam_max_y};
use the_nature_of_things::render::draw::{draw_weather_overlay, update_actor_positions, draw_minimap};
use the_nature_of_things::render::ui::{draw_panel, draw_controls, draw_pause_overlay};

fn window_conf() -> Conf {
    Conf {
        window_title:    "The Nature of Things — Cute Fantasy POC".to_owned(),
...
impl GameState {
    fn new(seed: u64) -> Self {
        ...
        let cx = (MAP_WIDTH  as f32 * TS * 0.5 - VIEWPORT_WIDTH  * 0.5).max(0.0);
        let cy = (MAP_HEIGHT as f32 * TS * 0.55 - SCREEN_HEIGHT  * 0.5).max(0.0);
        GameState {
            world,
            rng: StdRng::seed_from_u64(seed ^ 0xFAE_CAFE),
...
async fn main() {
    let textures = FantasyTextures::load().await;
    ...
    loop {
        ...
        if is_key_pressed(KeyCode::Tab) {
            ...
            if let Some(id) = gs.selected_actor {
                if let Some(a) = gs.world.actors.iter().find(|a| a.id == id) {
                    let wx = a.pixel_x / TILE_SIZE * TS;
                    let wy = a.pixel_y / TILE_SIZE * TS;
        ...
        if is_key_down(KeyCode::D) || is_key_down(KeyCode::Right) { gs.cam_x = (gs.cam_x + pan).min(cam_max_x()); }
        if is_key_down(KeyCode::W) || is_key_down(KeyCode::Up)    { gs.cam_y = (gs.cam_y - pan).max(0.0); }
        if is_key_down(KeyCode::S) || is_key_down(KeyCode::Down)  { gs.cam_y = (gs.cam_y + pan).min(cam_max_y()); }
        ...
        clear_background(Color::new(0.40, 0.65, 0.30, 1.0));
        draw_world_cutefantasy(&gs.world, &textures, gs.selected_actor, gs.cam_x, gs.cam_y);
        ...
        if gs.show_minimap {
            let mc_x = gs.cam_x / TS * TILE_SIZE;
            let mc_y = gs.cam_y / TS * TILE_SIZE;
            draw_minimap(&gs.world, mc_x, mc_y);
        }

        let label = format!(
            "Cute Fantasy Free  FPS:{:.0}  Day {}  {}  [WASD pan  Tab select  [/] speed]",
            get_fps(), gs.world.clock.day, gs.world.weather.label(),
        );
        draw_text(&label, 8.0, SCREEN_HEIGHT - 6.0, 14.0, Color::new(1.0,1.0,1.0,0.75));
        next_frame().await;
    }
}
