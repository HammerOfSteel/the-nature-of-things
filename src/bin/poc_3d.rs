/// POC: Macroquad 3D renderer (no extra crates — uses macroquad's Camera3D API).
/// Run with: cargo run --bin poc_3d
...
use the_nature_of_things::render::draw_3d::draw_world_3d;
use the_nature_of_things::render::draw::{update_actor_positions, draw_minimap};
use the_nature_of_things::render::ui::{draw_panel, draw_controls, draw_pause_overlay};

fn window_conf() -> Conf {
    Conf {
        window_title: "The Nature of Things — 3D POC".to_owned(),
...
impl GameState {
    fn new(seed: u64) -> Self {
        ...
        GameState {
            ...
            paused: false,
            show_minimap: false,  // minimap less useful in 3D view
        }
    }
...
async fn main() {
    ...
    loop {
        ...
        // Note: 3D click-to-select requires ray casting — deferred for now
        if is_mouse_button_pressed(MouseButton::Left) {
            let (mx, my) = mouse_position();
            // Rough tile pick from screen position (approximate for POC)
            if mx < VIEWPORT_WIDTH {
                let tx = ((mx + gs.cam_x) / TILE_SIZE) as i32;
                let ty = ((my + gs.cam_y) / TILE_SIZE) as i32;
                gs.selected_actor = gs.world.actors.iter()
                    .find(|a| a.tile_x == tx && a.tile_y == ty)
                    .map(|a| a.id);
            }
        }
        ...
        // ── Render ─────────────────────────────────────────────────────────────
        clear_background(Color::new(0.40, 0.52, 0.68, 1.0)); // sky colour behind 3D scene

        draw_world_3d(&gs.world, gs.selected_actor, gs.cam_x, gs.cam_y);

        // UI always in 2D (set_default_camera was already called inside draw_world_3d)
        ...
        let fps_str = format!("POC: 3D (macroquad)   FPS:{:.0}  Day {}  {}",
            get_fps(), gs.world.clock.day, gs.world.weather.label());
        draw_text(&fps_str, 4.0, 12.0, 10.0, Color::new(0.80, 0.80, 0.85, 0.90));
