/// Little Dreamyland pack POC — bunny characters + autotile ground + house sprites.
/// Run with: cargo run --bin poc_dreamyland
...
use the_nature_of_things::render::draw_dreamyland::{DreamyTextures, TS, draw_world_dreamyland, cam_max_x, cam_max_y};
...
fn window_conf() -> Conf {
    Conf {
        window_title:    "The Nature of Things — Little Dreamyland POC".to_owned(),
...
impl GameState {
    fn new(seed: u64) -> Self {
        ...
        GameState {
            world,
            rng: StdRng::seed_from_u64(seed ^ 0xD3EA_D1A5),
...
async fn main() {
    let textures = DreamyTextures::load().await;
    ...
    loop {
        ...
        clear_background(Color::new(0.38, 0.62, 0.32, 1.0));
        draw_world_dreamyland(&gs.world, &textures, gs.selected_actor, gs.cam_x, gs.cam_y);
        ...
        let label = format!(
            "Little Dreamyland  FPS:{:.0}  Day {}  {}  [WASD pan  Tab select  [/] speed]",
