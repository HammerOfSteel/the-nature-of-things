/// POC-5: Sunnyside World 16 px sprite tiles + layered animated characters.
/// Run with: cargo run --bin poc_sunnyside
...
fn window_conf() -> Conf {
    Conf {
        window_title:    "The Nature of Things — Sunnyside World POC".to_owned(),
...
async fn main() {
    ...
    loop {
        ...
        // ── Render ────────────────────────────────────────────────────────────
        clear_background(Color::new(0.36, 0.55, 0.30, 1.0));  // sunny sky-green bg
        ...
        let label = format!(
            "POC-5: Sunnyside World  FPS:{:.0}  Day {}  {}  [WASD pan  Tab select  [/] speed]",
            get_fps(), gs.world.clock.day, gs.world.weather.label(),
        );
        draw_text(&label, 4.0, 12.0, 10.0, Color::new(0.20, 0.15, 0.10, 0.90));
