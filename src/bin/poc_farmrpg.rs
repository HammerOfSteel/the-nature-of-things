/// Farm RPG 16×16 Tiny pack POC — spring tileset + assembled house + farmer walk.
/// Run with: cargo run --bin poc_farmrpg
...
use the_nature_of_things::render::draw_farmrpg::{FarmTextures, TS, draw_world_farmrpg, cam_max_x, cam_max_y};
...
fn window_conf() -> Conf {
    Conf {
        window_title:    "The Nature of Things — Farm RPG Tiny POC".to_owned(),
...
impl GameState {
    fn new(seed: u64) -> Self {
        ...
        GameState {
            world,
            rng: StdRng::seed_from_u64(seed ^ 0xFA3B_CAF3),
...
async fn main() {
    let textures = FarmTextures::load().await;
    ...
    loop {
        ...
        clear_background(Color::new(0.42, 0.62, 0.28, 1.0));
        draw_world_farmrpg(&gs.world, &textures, gs.selected_actor, gs.cam_x, gs.cam_y);
        ...
        let label = format!(
            "Farm RPG Tiny  FPS:{:.0}  Day {}  {}  [WASD pan  Tab select  [/] speed]",
